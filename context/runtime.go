...
type Runtime struct {
	...
	User models.User
	ActivityStream chan<- activity.Message
}
...
// integer.
func (runtime *Runtime) ParamInt(name string) (int, error) {
...
func (runtime *Runtime) Publish(message activity.Message) {
	runtime.ActivityStream <- message
}

// RequestHeader
// 
// helper function to return a request header from the request.
func (runtime *Runtime) RequestHeader(name string) string {
...
func (runtime *Runtime) AddResult(result Result) {
	runtime.Results = append(runtime.Results, result)
}

// SetMeta
func (runtime *Runtime) AddMeta(key string, value interface{}) {
...
// PersistFile
func (runtime *Runtime) PersistFile(target File, mime string) (models.File, error) {
...
// Blueprint
func (runtime *Runtime) Blueprint() Blueprint {
